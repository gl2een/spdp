...
def eval(words, postags, heads_pred, types_pred, heads, types, word_alphabet, pos_alphabet, lengths,
    ...
    batch_size, _, lemma_length = words.shape
    ucorr = 0.
    lcorr = 0.
    total = 0.
    ucomplete_match = 0.
    ...
    for i in range(batch_size):
        ...
        for j in range(start, lengths[i] - end):
            word = []
            pos = []
            for k in range(lemma_length):
                if words[i, j, k] == 1:
                    break
                word.append(word_alphabet.get_instance(words[i, j, k]))
                pos.append(pos_alphabet.get_instance(postags[i, j, k]))
            word = " ".join(word)
            pos = "+".join(pos)
            #word = word_alphabet.get_instance(words[i, j]) delete 180809
            #word = word.encode('utf8') delete 180809

            #pos = pos_alphabet.get_instance(postags[i, j]) delete 180809
            #pos = pos.encode('utf8') delete 180809
...
def eval_get_error_data(words, postags, heads_pred, types_pred, heads, types, word_alphabet, pos_alphabet, lengths,
         punct_set=None, symbolic_root=False, symbolic_end=False):
