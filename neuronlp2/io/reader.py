...
class CoNLLXReader(object):
    ...
    def getNext_predict(self, normalize_digits=True, symbolic_root=False, symbolic_end=False, sent_id=0):
        """
        190614 gl2een
        read next dependency parsed sentence without head, type for prediction
        """
        line = self.__source_file.readline()
        # skip multiple blank lines.
        while len(line) > 0 and len(line.strip()) == 0:
