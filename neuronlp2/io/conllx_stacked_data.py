...
def read_stacked_data_for_prediction(source_path, word_alphabet, char_alphabet, pos_alphabet, type_alphabet, max_size=None, normalize_digits=True, prior_order='deep_first'):
    data = [[] for _ in _buckets]
    max_lemma_length = [0 for _ in _buckets]
    max_char_length = [0 for _ in _buckets]
    print('Reading data from %s' % source_path)
...
def read_stacked_data_to_variable_for_prediction(source_path, word_alphabet, char_alphabet, pos_alphabet, type_alphabet,
                                  max_size=None, normalize_digits=True, prior_order='deep_first', use_gpu=False):
...
def iterate_batch_stacked_variable_for_prediction(data, batch_size, unk_replace=0., shuffle=False, use_gpu=False):
    """
    190614 gl2een
    for prediction
    :param data:
    ...
    for bucket_id in bucket_indices:
        ...
        if bucket_size == 0:
            continue
        data_encoder, data_sentences = data_variable[bucket_id]
        words, chars, pos, masks_e, single, lengths_e = data_encoder
        lemma_length = words.size(2)
        if unk_replace:
            ones = Variable(single.data.new(bucket_size, bucket_length, lemma_length).fill_(1))
            noise = Variable(masks_e.data.new(bucket_size, bucket_length, lemma_length).bernoulli_(unk_replace).long())
        ...
        for start_idx in range(0, bucket_size, batch_size):
            if shuffle:
                ...
                excerpt = slice(start_idx, start_idx + batch_size)
            if use_gpu:
                yield (words[excerpt].cuda(), chars[excerpt].cuda(), pos[excerpt].cuda(), masks_e[excerpt].cuda(), lengths_e[excerpt].cuda()), \
                      data_sentences[excerpt]
            else:
                yield (words[excerpt], chars[excerpt], pos[excerpt], masks_e[excerpt], lengths_e[excerpt]), \
                      data_sentences[excerpt]
